...
main ( int argc, char **argv )
{
    ...
    for ( int_t iteration = 0; iteration <= max_iteration; iteration++ )
    {
        ...
        if ( iteration % snapshot_frequency == 0 )
        {
            ...
            cudaMemcpy(h_mass[1], d_mass_1, (N+2) * sizeof(real_t), cudaMemcpyDeviceToHost);

            cudaMemcpy(h_mass_velocity_x[0], d_mass_velocity_x_0, (N+2) * sizeof(real_t), cudaMemcpyDeviceToHost);
            cudaMemcpy(h_mass_velocity_x[1], d_mass_velocity_x_1, (N+2) * sizeof(real_t), cudaMemcpyDeviceToHost);
